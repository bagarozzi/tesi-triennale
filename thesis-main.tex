...
Infine, verrà presentato un semplice test di funzionamento del cluster e le conclusioni con i possibili sviluppi futuri del progetto ed i confronti con altre implementazioni motivando le differenze.
...
\begin{itemize}
    ...
    \item {l'impostazione di \texttt{Systemd} come cgroup manager per \texttt{containerd} e Kubernetes.
        \lstinputlisting[language=bash,label={lst:systemd}, firstline=37, lastline=37]{listings/master.yaml}
    }
    \item {l'attivazione di \textit{ip forwading} per permettere ai nodi di scambiare pacchetti tra di loro.
        \lstinputlisting[language=bash,label={lst:ip-fwd}, firstline=40, lastline=40]{listings/master.yaml}
    }
...
\lstinputlisting[language=bash, label={lst:join-script}, caption=Script di integrazione dei nodi worker]{listings/join-nodes.sh}
...
\begin{itemize}
    ...
    \item {\textbf{Esposizione tramite NodePort}: un metodo decisamente più semplice che consiste in impostare l'Ingress Controller in modalità NodePort.
        ...
        A questo fine sarà sufficiente applicare la configurazione del controller:
        \lstinputlisting[language=bash, label={lst:nginx-ctl}]{listings/nginx-ctl.sh}
        }
\end{itemize} 
\chapter{Testing}
