...
\begin{abstract}	
Questo elaborato descrive un progetto di automazione del dispiegamento di un cluster Kubernetes su infrastruttura OpenStack, sviluppato nel contesto del progetto Nereide dell'Istituto Nazionale di Geofisica e Vulcanologia (INGV). L'obiettivo è semplificare l'installazione di ambienti computazionali complessi attraverso script Infrastructure-as-Code (IaC), per agevolare l'uso di Virtual Data Center (VDC).

Dopo un'analisi delle tecnologie utilizzate (OpenStack, Kubernetes ed Infrastructure-as-Code), viene illustrata l'implementazione pratica: il provisioning delle macchine virtuali tramite interfaccia Horizon e la creazione del cluster.
Ulteriore attenzione è data alle varie possibilità di automazione che si possono attuare: dall'automazione del provisioning a quella della creazione del cluster.
Il funzionamento del cluster viene verificato con un deployment di prova basato su un server web Nginx, che espone due servizi web accessibili tramite routing Ingress.

Infine vengono proposti anche degli sviluppi futuri per il consolidamento del progetto, tra cui: l'integrazione di sistemi di monitoraggio, la ridondanza dei nodi master per aumentare la resilienza e l'adozione di buone pratiche per la manutenzione del cluster.
\end{abstract}
...
ciò che succede ha sul clima della terra.
